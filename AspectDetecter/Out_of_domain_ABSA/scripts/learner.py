...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(model.state_dict(), '/Users/jordanharris/SCAPT-ABSA/AspectDetecter/Out_of_domain_ABSA/models/' + self.cur_date + '_state_dict.pt')
        torch.save(model, '/Users/jordanharris/SCAPT-ABSA/AspectDetecter/Out_of_domain_ABSA/models/' + self.cur_date + '_model.pth')
...
class Trainer:
    ...
    def fit(self, num_epochs, args, device='cpu:0'):
        """
    ...
    def optimizer(self, args):
        ...
        optimizer = AdamW([
            ...
            {'params': self.model.ffn.parameters(),
             'lr': float(vars(args)['ffn_lr'])},
        ], lr=float(vars(args)['bert_lr']), correct_bias=True)
        num_train_steps = (int(len(self.train_data_loader.dataset)) /
                           int(vars(args)['train_batch_size'])) * int(vars(args)['max_epoch'])
...
class EvaluateOnTest:
    ...
    def predict(self, device='cpu', pbar=None):
        """
        Evaluate the models on a validation set
        :param device: str (defaults to 'cuda:0')
        :param pbar: fast_progress progress bar (defaults to None)
