...
class Trainer(object):
    ...
    def fit(self, num_epochs, args, device='mps:0'):
        """
    ...
    def predict(self, device='mps:0', pbar=None):
        """
        ...
        preds_dict = {
            'y_true': np.zeros([current_size, 7]),
            'y_pred': np.zeros([current_size, 7])
        }
    ...
    def predict(self, device='mps:0', pbar=None):
        ...
        with torch.no_grad():
            ...
            with open('predict.csv', 'w') as f:
                writer = csv.writer(f)
                writer.writerow(
                    ['ID', 'text', 'food', 'restaurant', 'service', 'atmosphere', 'drinks', 'location', 'None'])
